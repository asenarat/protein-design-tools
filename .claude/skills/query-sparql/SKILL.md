...
**Publications**: `?repo schema:citation ?doi` is **sparse** (664 total, 368 repos). Richer: papers authored by people — `?doi schema:author ?orcid` (347 papers, 154 ORCID authors), with `schema:name`, `schema:datePublished`, `op:infoscienceArticleIdentifier`. To find a tool's "related publications", map its contributors' ORCIDs and pull the papers they authored.
