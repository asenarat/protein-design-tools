---
name: frontend-dev
description: Frontend / UI work on the pulseWebKit dashboard (SvelteKit + Tailwind v4). Encodes the SDSC visual identity (Swiss Data Science Center UI Kit v2.1) translated into a permanent dark theme — colors, typography, spacing, components, and page layouts the dashboard must follow. TRIGGER when editing anything under `dashboard/src/**/*.svelte`, `dashboard/src/app.css`, or `dashboard/static/`; adding or restyling a page, component, or design token; rewriting layouts; changing colors, fonts, radius, or spacing; reviewing UI PRs; or when the user says "design", "restyle", "rebrand", "dark mode", "match the Figma", "make it pretty". SKIP for backend, devcontainer, Docker, CI, or shell-script work.
---

# Frontend dev — pulseWebKit visual system (dark-mode SDSC)

> **Source of truth for UI work in this repo.** Read this before touching anything in `dashboard/`. The pulseWebKit dashboard runs as a **permanent dark theme**: it adopts the Swiss Data Science Center UI Kit v2.1 (Figma Make: `ui-kit-basic--Community-`, file key `xFRPysbb2ni4RXyuT7dVqq`, based on `datascience.ch`) — its typography, spacing, sharp corners, button shape, and `〇` label pattern — and translates the kit's palette into ink-blues on a near-black canvas. There is **no light mode**; do not add one.
>
> This skill replaces the legacy `dashboard/DESIGN_SYSTEM.md`. When the two disagree, this file wins; the old file is a migration reference only and is to be deleted after the refactor in §10 lands.
...
2. Cross-reference `CLAUDE.md` for general repo conventions (Svelte 5 runes, MSW, type-check gate) and the **UI verification rule** (every visible change must be exercised through the Playwright MCP server against `http://localhost:5173`).
...
The dashboard is **dark, Swiss, sharp-edged**:
...
- **All-caps Switzer buttons** with `rounded-none`, and the `〇`-prefixed labelling pattern from the Figma kit.

Translation rule from the (light-mode) Figma kit to this dark theme:

| Figma kit (light) | This dashboard (dark) |
|---|---|
...
All design tokens live in `dashboard/src/app.css`, in **both** the Tailwind v4 `@theme` block (generates `bg-op-*`, `text-op-*`, `border-op-*` utilities) and the `:root` block (for inline styles and D3 code that can't use Tailwind).
...
Source: `src/lib/components/graph/graphData.ts` (`NODE_COLORS`). All viz code reads from there — no second hex table.
...
1. Add `--color-op-<name>: <hex>;` to `@theme` in `src/app.css`.
2. Add `--op-<name>: <hex>;` to `:root` in `src/app.css`.
3. Document it in §2 of this file.
4. Never write a raw hex in Svelte template markup. D3/SVG drawing code is the only exception.
...
| **JetBrains Mono** | Code, IDs, SHA fragments, Cypher snippets — anything monospaced. Keep the `.mono` utility class. | Fontsource (`@fontsource/jetbrains-mono`) |

All three fonts are loaded as **JavaScript imports** at the top of `src/routes/+layout.svelte` (Vite resolves npm font packages from JS context; importing bare npm specifiers in `app.css` under `@tailwindcss/vite` does not work):
...
`src/app.css` contains **no** font `@import` statements — only `@import 'tailwindcss'` and the `@theme` / `:root` token blocks.

Set `'Switzer', system-ui, sans-serif` on `body`. Set `'Space Grotesk Variable', 'Space Grotesk', system-ui, sans-serif` on `h1`–`h6`. D3/SVG text elements require an explicit `font-family` attribute — they do not inherit from CSS.
...
Apply `.mono` for any monospaced content (IDs, Cypher, JSON dumps).
...
<div class="w-12 h-12 flex items-center justify-center rounded-none"
  ...
  <YourIcon class="w-6 h-6" style="color:var(--op-blue-light)" />
</div>
```
...
<section class="grid md:grid-cols-2 gap-0 border border-op-border">
  <div class="h-96"
       style="background:url(/static/patterns/dots-left.svg) center/cover;
...
This is the template the **Pipeline** and **Health** routes should adopt to replace the current `AppShell`.

### 7.9 Event index / pipeline run row

The kit's event row pattern (date chip + title + meta line + CTA) is the direct analogue of the Pipeline Run list item. The date chip uses `bg-op-blue` text white (or `bg-op-blue-dark` for past runs).
...
Since the whole site is dark, the Graph Explorer no longer needs an "exception" to the theme — it shares the same `--op-bg` and `--op-surface` tokens. Specifics:

- **Canvas surface** uses the page background `--op-bg` (`#0c0c12`) with a subtle dot grid (`radial-gradient(circle, #242438 1px, transparent 1px)` at `28px 28px`). Class: `.dot-grid` (already defined in `src/app.css`).
...
- **D3 link colour**: `var(--op-blue)` (`#5461a6`). Stored as a hex literal in D3 code since D3 can't read CSS variables.
- **Selection ring** around an active node: `var(--op-blue-light)` (`#8a94c9`) at 80% opacity.
- **Node fill colours** (Person/Repo/Commit/Org/PR): from `NODE_COLORS` in `graphData.ts` — never duplicate the hex table.
...
- **Hero / banner pattern**: optional dotted SVG/PNG pattern in `static/patterns/`. Anchor `background-position: left center` or `right center`, never `center` — asymmetric placement is intentional. Layer over `--op-blue-dark` with `background-blend-mode: overlay` so the pattern remains visible but dim.
...
## 10. Migration from the legacy dark theme

The current dashboard already runs dark, so most foundational work (page-bg, surface, text) stays. The refactor is mostly about **shape, font, button casing, and dropping the brand-green primary**:

1. **Tokens** — rewrite `src/app.css` `@theme` + `:root` to the §2 palette. Drop `--color-op-green*` and the legacy `--color-op-nav*` (now unified under blue tokens). Keep `--op-bg`, `--op-surface*`, `--op-border*` essentially as they are, with minor hex tweaks (per §2.1).
2. **Fonts** — add Switzer `@import` (Fontshare). Keep Space Grotesk + JetBrains Mono. Set Switzer as the `body` default and Space Grotesk on `h1`–`h6`.
3. **Radius purge** — find/replace `rounded-xl`, `rounded-lg`, `rounded-md` → `rounded-none` in `dashboard/src/**/*.svelte` (excluding `rounded-full` for avatars / floating icon buttons).
4. **Brand-green removal** — search for `--op-green*`, `bg-op-green*`, `text-op-green*`, `glow-green`, hex `#90ca42` / `#6a9830`. The SDSC system has **no chrome green** — replace primary actions with `--op-blue`, success states with `--op-success`. The blinking cursor (`.cursor::after`) keeps green or moves to `--op-blue-light` — your call, but pick one.
5. **Button overhaul** — uppercase text, `tracking-wide`, `rounded-none`, Switzer; new variants per §6.2. Replace `style="background:var(--op-green);color:#0d1a00"` with the primary-button pattern. Remove any `glow-*` classes from chrome buttons.
6. **AppShell** — port to the §7.8 "content + sidebar" pattern (dark sidebar `--op-surface`, active item with blue-light text + left border). Drop the old `--op-nav` purple sidebar tokens.
7. **Pipeline Runs** — list rows use the §7.9 event pattern (date chip in blue, dark surface body).
8. **Health page** — service tiles use the §7.4 3- or 4-column feature grid with the §6.6 square icon box.
9. **Graph page** — chrome (header, query sidebar, node-detail panel, timeline strip) adopts the standard dark-SDSC surfaces. Canvas + overlays remain dark per §8.
10. **Footer** — add the §7.2 black SDSC footer with partner logos to every route, including `/graph`.
11. **Cleanup** — delete `dashboard/DESIGN_SYSTEM.md`. Update `CLAUDE.md` and `.claude/PROJECT.md` references from `DESIGN_SYSTEM.md` → this skill (`.claude/skills/frontend-dev/SKILL.md`).

Each step should be a verifiable change with a Playwright MCP screenshot (per the UI-verification rule in `CLAUDE.md`).
...
- Don't add glow / neon / drop-shadow chrome — they're not SDSC. The legacy `.glow-green` / `.glow-purple` utilities should be deleted.
- Don't make button text Title Case or lowercase. **UPPERCASE only.**
- Don't write raw hex in Svelte template markup. Use `var(--op-*)` or `bg-op-*` utilities (D3/SVG drawing is the only exception).
- Don't add a light-mode toggle. The dashboard is dark by design.
